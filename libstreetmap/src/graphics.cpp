 ...
 */
#define FONT_CACHE_SIZE_FOR_ZEROS 10000 //40
#define FONT_CACHE_SIZE_FOR_ROTATED 100000 //100
...
handle_zoom_in(float x, float y, void (*drawscreen) (void)) {
    if (display_help == false) {
        float area_threshold = 3.5e-09;

        bool new_area_in_bounds = LOD_area_test(area_threshold);

        if (!new_area_in_bounds) {
...
int xworld_to_pixel(float worldx) {
    return (int) (xworld_to_pixel_fl(worldx) + 0.5);
}

float xworld_to_pixel_fl(float worldx) {
    float winx;
 ...
 * (pixels) in the y direction.  Add 0.5 at end for extra half-pixel accuracy. 
 */
int yworld_to_pixel(float worldy) {
    return (int) (yworld_to_pixel_fl(worldy) + 0.5);
}

float yworld_to_pixel_fl(float worldy) {
    float winy;
